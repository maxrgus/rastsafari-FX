...
public class StorageDB implements Storage {
	...
	public ObservableList<SafariLocation> getLocationsFromStorage() {
		...
		try {
			...
			while (rs.next()) {
				locations.add(new SafariLocation(rs.getInt("id"), rs.getString("locationName"),
						rs.getString("description"), rs.getString("location"), rs.getInt("minParticipants"),
						rs.getInt("maxParticipants"), rs.getInt("isActive")));
			}
			rs.close();
		...
		try {
			...
			while (rs.next()) {
				customers.add(new Customer(rs.getInt("id"), rs.getString("givenName"), rs.getString("familyName"),
						rs.getString("SSID"), rs.getString("email"), rs.getString("phoneDay"),
			...
			while (rs.next()) {
				bookings.add(new Booking(rs.getInt(1),
						...
						new Safari(rs.getInt(10),
								new SafariLocation(rs.getInt(11), rs.getString(12), rs.getString(13), rs.getString(14),
										rs.getInt(15), rs.getInt(16), rs.getInt(17)),
								rs.getString(18), rs.getString(19), rs.getString(20), rs.getInt(21),
								new Guide(rs.getInt(22), rs.getString(23), rs.getString(24), rs.getString(25)))));
			}
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
